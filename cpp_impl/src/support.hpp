...
double dist_torus(vector<double>& p1, vector<double>& p2, vector<int>& bounds) {
    double sum = 0.0;
    for (int i = 0; i < min(p1.size(), p2.size()); i++) {
...
void update_dist_mat(PointCloud& pc, vector<double>& dmat, vector<int>& bounds) {
    int n_pts = pc.size();
...
Support compute_support(DynPointCloud& dpc, vector<int>& bounds) {
    int n_time = dpc.size();
