...
if __name__ == "__main__":
    ...
    if not args.no_analysis:
        ...
        # ax.scatter(X_cmds[:, 0], X_cmds[:, 1], X_cmds[:, 2], c=colors)
        plt.scatter(X_cmds[:, 0], X_cmds[:, 1], c=colors)
        plt.show()

        Z = linkage(squareform(dist_mat), 'single')
        dn = dendrogram(Z, color_threshold=0, above_threshold_color="black", leaf_label_func = lambda i : 1+i//flocks)
        
        xcoords = []
        for (x1, _, _, x2), (y1, _, _, y2) in zip(dn["icoord"], dn["dcoord"]):
            if y1 == 0: xcoords.append(x1)
            if y2 == 0: xcoords.append(x2)
        xcoords.sort()

        for coord_i, color_i in enumerate(dn["leaves"]):
            plt.plot(xcoords[coord_i], 0, color=colors[color_i], marker="d", markersize=8)
